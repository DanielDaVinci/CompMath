...
class MandelbrotContourDraw(MandelbrotDraw):
    ...
    def _find_contours(self, image_bin: np.ndarray):
        contours_pix, _ = cv2.findContours(image=image_bin.astype(np.uint8), mode=cv2.RETR_LIST,
        ...
        for contour_pix in contours_pix:
            ...
            subset = (contour_pix[0] >= pad) & (contour_pix[1] >= pad) & (contour_pix[0] < w - pad) & (
                    contour_pix[1] < h - pad)
        ...
        # Преобразование\_ пикселей(исходные координаты найденных контуров) в комплексную плоскость
